...
DEPENDENCES="dbus gdb ghostscript gjs highway libelf libgudev libunwind luajit nspr python python-gobject qoi-headers-git fcitx5" #SYNTAX: "APP1 APP2 APP3 APP4...", LEAVE BLANK IF NO OTHER DEPENDENCIES ARE NEEDED
...
_extract_package() {
	...
	if test -f "$pkg_full_path"; then
		...
		[ -n "$lib_browser_launcher" ] && [[ "$arg" =~ (fcitx5|babl-git|gegl-git|hicolor-icon-theme|xapp|dbus|gdb|ghostscript|gjs|libunwind|luajit|nspr|python|python-gobject|libgudev) ]] && tar fx "$pkg_full_path" -C ./base/ --warning=no-unknown-keyword --exclude='.PKGINFO'
	fi
}
